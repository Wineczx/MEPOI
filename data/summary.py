...
def generate_summary(texts):
    ...
    for text in texts:
        ...
        if word_count(text) > 20:
            # Add the prompt
            prompt = f"Summarize the text as a user comment on the place in a sentence of 20 words or less, ensuring that the resulting text is a complete sentence of less than 20 words in length, and can omit unimportant content:{text}"
            ...
            summaries.append(text)  # Return the original text if the word count is less than or equal to 20
    return summaries
...
# 读取原始数据集
df = pd.read_json('/data/CaiZhuaoXiao/yelp/FL/FL_reviews_summary4.json', lines=True)
...
with open('/data/CaiZhuaoXiao/yelp/FL/FL_reviews_summary5.json', 'a') as outfile:
    for i in tqdm(range(0, num_samples, batch_size), position=0, leave=True):
        ...
        if set(batch_df['review_id']).issubset(processed_ids):
            continue
        ...
        batch_df['summary'] = batch_summaries  # Here we avoid using .loc for assigning to avoid the warning
